...
export default function Page() {
  ...
  const launchGame = async (gameId: number, triggeredByPolice = false) => {
    // Show Rick Roll for Tilt Master
    if (gameId === 1 && !triggeredByPolice) {
      ...
      // Wait 3 seconds then launch the game
      setTimeout(() => {
  ...
  const actuallyLaunchGame = async (gameId: number, triggeredByPolice = false) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to launch game. Make sure the backend is running on port 8000.',
        variant: 'destructive',
      })
    } finally {
      setLoading(null)
  ...
  // Police mode will trigger random game when bad posture detected
  // The posture detection will be implemented by external system
  // This function can be called when bad posture is detected
  const handleBadPosture = () => {
    if (policeModeEnabled) {
      const randomGame = Math.floor(Math.random() * 2) // 0 or 1
      launchGame(randomGame, true)
    }
  }

  // Expose handleBadPosture to window so external posture detection can call it
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-4xl space-y-12">
        ...
        <div className="flex justify-center">
          <Button
            onClick={() => setPoliceModeEnabled(!policeModeEnabled)}
